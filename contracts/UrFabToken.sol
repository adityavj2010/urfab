...
contract UrFabToken is ERC20, Owned, SafeMath {
    ...
    event TransferDetails(address from, address to, uint256 tokens);
    ...
    function transferFrom(
        ...
        emit TransferDetails(from, to, tokens);
...
contract UrfabCode is UrFabToken {
    enum Roles {
        Inactive,
        Active
    }
    ...
    modifier onlyRegisteredParticipant() {
        // require(membership[msg.sender] == 1,'User not a member');
        _;
    }
    ...
    function request(uint256 productId, uint256 hash)
        public
        onlyRegisteredParticipant
    {
    ...
    function response(
        ...
        uint256 productCost,
        uint256 productCode
    ) public payable onlyRegisteredParticipant {
        // require(products[productId].currentOwner == msg.sender);
        if (products[productId].hashOfDetails == hash) {
            products[productId].status = state;

            if (state == productState.Sold) {
                this.transferFrom(products[productId].buyer, products[productId].currentOwner, products[productId].productCost);
            }
    ...
    event Debugger(address key, address value);
